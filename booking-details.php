...
<th>Number of members</th>
 <td id="noOfMembers"><?php echo htmlentities($row->NumberOfMembers);?></td>  
<th>User Remark</th>
...
<tr>
  ...
  <td colspan="2"><?php
    $totAmtQuery = "SELECT `tblbookings`.`NumberOfMembers` * `tblevents`.`EventPrice` AS `totAmt` FROM `tblbookings` JOIN `tblevents` ON `tblevents`.`id` = `tblbookings`.`EventId` WHERE `tblbookings`.`id` = :bkId;";
    $totQuery = $dbh -> prepare($totAmtQuery);
    $totQuery->bindParam(':bkId',$_GET['bkid'],PDO::PARAM_STR);
    $totQuery->execute();
    $totResult=$totQuery->fetchAll(PDO::FETCH_COLUMN);
